#!/usr/bin/env python3
"""
Single-frame Task 2 comparison figure (velocity_from_interface).

For one BulkFlow index (--frame), runs inference with every model in
inference_metrics_task2.MODEL_REGISTRY and saves a two-row panel:
  Row 0: SDF | GT temperature | [models' temperature]
  Row 1: Interface |V| | GT bulk |V| | [models' |V|]

No physics metrics are computed or drawn on the figure.

Usage:
    python scripts/generate_task2_figures.py --frame 100
    python scripts/generate_task2_figures.py --frame 100 --models fm hbarfm
    python scripts/generate_task2_figures.py --frame 100 --no-range-titles
"""
...
def _run_single_model_frame(
    ...
    """Returns (pred_temp_H_W, pred_vel_mag_H_W) in physical units, or None."""
    ...
    try:
        if is_ar_bootstrap:
            ...
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(
                ...
                _, _, _, pred_velx, pred_vely, pred_temp = (
                    run_ar_bootstrap_inference_batch(
                        model,
                        dataset,
                        device,
            ...
            pred_velx = pred_velx[0]
            pred_vely = pred_vely[0]
            ...
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(
                ...
                _, _, _, pred_velx, pred_vely, pred_temp = run_inference_batch(
                    model,
                    dataset,
                    device,
                    args.num_inference_steps,
        ...
        vel_mag = np.sqrt(pred_velx**2 + pred_vely**2)
        return pred_temp, vel_mag
    except Exception as e:
...
def generate_task2_panel(args: argparse.Namespace) -> str:
    ...
    gt_velx = ref_ds._denormalize_field(target_data[target_channels[0]].clone(), "velx").numpy()
    gt_vely = ref_ds._denormalize_field(target_data[target_channels[1]].clone(), "vely").numpy()
    gt_temp = ref_ds._denormalize_field(target_data[target_channels[2]].clone(), "temperature").numpy()
    gt_vel_mag = np.sqrt(gt_velx**2 + gt_vely**2)

    sdf = ref_ds._denormalize_field(input_data[0].clone(), "sdf").numpy()
    velx_interface = ref_ds._denormalize_field(input_data[1].clone(), "velx_interface").numpy()
    vely_interface = ref_ds._denormalize_field(input_data[2].clone(), "vely_interface").numpy()
    interface_vel_mag = np.sqrt(velx_interface**2 + vely_interface**2)

    temp_vmin_phys, temp_vmax_phys = float(gt_temp.min()), float(gt_temp.max())
    vel_vmax = float(gt_vel_mag.max())
    interface_vel_vmax = float(interface_vel_mag.max())
    ...
    width_ratios = [1, 1, args.col_gap] + [1] * n_models
    ...
    def plot_vel(ax, data: np.ndarray, show_range: bool) -> None:
        nonlocal last_vel_im
        norm = PowerNorm(gamma=0.5, vmin=vel_vmin, vmax=vel_vmax)
        im = ax.imshow(data, cmap="turbo", norm=norm, origin="lower")
        ax.axis("off")
        last_vel_im = im
        if show_range and args.range_titles:
            ax.set_title(
    ...
    def plot_interface_vel(ax, data: np.ndarray) -> None:
        norm = PowerNorm(gamma=0.5, vmin=vel_vmin, vmax=interface_vel_vmax)
        ax.imshow(data, cmap="turbo", norm=norm, origin="lower")
        ax.axis("off")
    ...
    ax_vint = fig.add_subplot(gs[1, 0])
    plot_interface_vel(ax_vint, interface_vel_mag)
    ax_vint.text(
        0.5,
        args.label_offset,
        "Interface Vel.",
    ...
    ax_gt_t = fig.add_subplot(gs[0, 1])
    plot_temp(ax_gt_t, gt_temp, show_range=True)
    ax_gt_t.text(
        0.5,
        args.label_offset,
        "Ground Truth",
        transform=ax_gt_t.transAxes,
        fontsize=label_fontsize,
        fontweight="bold",
        ha="center",
    )

    ax_gt_v = fig.add_subplot(gs[1, 1])
    plot_vel(ax_gt_v, gt_vel_mag, show_range=True)
    ax_gt_v.text(
        0.5,
        args.label_offset,
        "Ground Truth",
        transform=ax_gt_v.transAxes,
        fontsize=label_fontsize,
        fontweight="bold",
        ha="center",
    )

    for i, model_name in enumerate(models_to_run.keys()):
        col = i + 3
        ...
        if pred is not None:
            pt, pv = pred
            plot_temp(ax_t, pt, show_range=True)
            plot_vel(ax_v, pv, show_range=True)
        else:
            ax_t.axis("off")
            ax_v.axis("off")
        ax_t.text(
    ...
    if last_temp_im is not None:
        cax_t = fig.add_axes([cbar_left, 0.53, args.cbar_width, 0.35])
    ...
    if last_vel_im is not None:
        cax_v = fig.add_axes([cbar_left, 0.12, args.cbar_width, 0.35])
        cbar_v = fig.colorbar(last_vel_im, cax=cax_v)
        cbar_v.ax.tick_params(labelsize=args.cbar_tick_fontsize)
        cbar_v.set_label("m/s", fontsize=args.cbar_fontsize)

    os.makedirs(args.output_dir, exist_ok=True)
    out_path = os.path.join(args.output_dir, f"task2_panel_frame{frame_idx}.png")
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Task 2 single-frame multi-model comparison figure (no metrics).",
    ...
    p.add_argument("--output-dir", type=str, default="./ICML/CamReady/Figure3_Task2")
    p.add_argument("--normalization-stats", type=str, default=DEFAULT_NORMALIZATION_STATS)
    p.add_argument("--norm-mode", type=str, default="all", choices=["none", "all", "temperature_only"])
    ...
    p.add_argument("--vel-vmin", type=float, default=0.2)
    ...
    p.add_argument(
        ...
        default=20,
        dest="range_fontsize",
        help="Font size for [min,max] numeric titles above temperature/velocity panels.",
    )
    p.add_argument(
        "--cbar-fontsize",
        type=int,
        default=18,
        dest="cbar_fontsize",
        help="Font size for colorbar axis labels (°C, m/s).",
    )
    p.add_argument(
        "--cbar-tick-fontsize",
        type=int,
    ...
    p.add_argument("--col-gap", type=float, default=0.05, dest="col_gap")
    p.add_argument("--fig-width", type=float, default=32.0)
    ...
    p.add_argument(
        ...
        default=True,
        help="Show [min,max] above temperature/velocity panels (default: on). "
        "Pass --no-range-titles to hide them.",
    )
    args = p.parse_args()
    generate_task2_panel(args)
